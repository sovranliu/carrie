...
import com.dianping.midasx.base.json.JSONObject;
...
public class RootConfig extends Config implements IRootConfig {
    ...
    public boolean attach(String path, IConfig conf) {
        ...
        ((com.dianping.midasx.utility.config.Config) (visit(path.substring(0, i)))).children.put(path.substring(i + 1), new Set<IConfig>(conf));
        return true;
    }
        ...
        try {
            ...
            if(!Text.isBlank(data)) {
                ...
                for(ILink<String, IJSON> link : jObject) {
                    conf.properties.put(link.origin(), link.destination().toString());
                }
            }
        }
        catch (Exception e) {
